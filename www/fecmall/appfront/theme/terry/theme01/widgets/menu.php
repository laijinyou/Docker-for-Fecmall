...
<div class="hidden" id="h5-menu">
	...
	<?php if (is_array($categoryArr) && !empty($categoryArr)) : ?>
		<div id="products-menu-l1" class="text-sm hidden">
			<div id="products-menu-l1-back" class="flex items-center py-3 px-2 mb-4 border-b-2">
			...
			<?php foreach ($categoryArr as $category1) : ?>
				<?php $childMenu1 = isset($category1['childMenu']) ? $category1['childMenu'] : null;   ?>
				<?php if (is_array($childMenu1) && !empty($childMenu1)) : ?>
					<div id="<?= $category1['_id'] ?>">
						<a href="javascript:void(0);" onclick="showProductsLevelMenu(1, <?= $category1['_id'] ?>)">
						...
						<div id="products-menu-l2" class="hidden">
							<div id="products-menu-l1-back" class="flex items-center py-3 px-2 mb-4 border-b-2">
								<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-red-500 font-bold" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
									<path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
								</svg>
								<div class="text-xs w-full">后退2</div>
							</div>
							<?php foreach ($childMenu1 as $category2) : ?>
								<?php $childMenu2 = isset($category2['childMenu']) ? $category2['childMenu'] : null;   ?>
								<?php if (is_array($childMenu2) && !empty($childMenu2)) : ?>
									<div id="<?= $category2['_id'] ?>">
										<a href="javascript:void(0);" onclick="showProductsLevelMenu(2, <?= $category2['_id'] ?>)">
											<div class="px-4 py-2 flex justify-between">
										...
										<div id="menu-h5-level4" class="hidden">
											<div id="products-menu-l1-back" class="flex items-center py-3 px-2 mb-4 border-b-2">
												...
												<div class="text-xs w-full">后退3</div>
											</div>
											<?php foreach ($childMenu2 as $category3) : ?>
												<div><a href="<?= $category3['url'] ?>" class=""><div class="px-4 py-2"><?= $category3['name'] ?></div></a></div>
											<?php endforeach; ?>
											<div class="border-t-2 mt-4"><a href=""><div class="text-center p-6 text-blue-800 text-xs font-bold">阅读全部</div></a></div>	
										</div>
									</div>
								<?php else: ?>
									<div><a href="<?= $category2['url'] ?>" class=""><div class="px-4 py-2"><?= $category2['name'] ?></div></a></div>
								<?php endif; ?>
							<?php endforeach; ?>
							<div class="border-t-2 mt-4"><a href=""><div class="text-center p-6 text-blue-800 text-xs font-bold">阅读全部</div></a></div>
						</div>
				...
				<?php else: ?>
					<div><a href="<?= $category1['url'] ?>" class=""><div class="px-4 py-2"><?= $category1['name'] ?></div></a></div>	
				<?php endif; ?>
			<?php endforeach; ?>
			<div class="border-t-2 mt-4"><a href=""><div class="text-center p-6 text-blue-800 text-xs font-bold">阅读全部</div></a></div>
		</div>
